...
void maxpath(int* maxPath, pid_t* who, pid_t ig)
{
    int i;
    int path;
    int procNumber;
    ...
    for(i = 0; i < NR_PROCS; ++i)
    {
          ...
          mproc[i].mp_pid != ig)
      {
          path = 0;
          procNumber = i;
          while((mproc[mproc[procNumber].mp_parent].mp_flags
                & IN_USE) &&
                mproc[mproc[procNumber].mp_parent].mp_pid != ig &&
                mproc[mproc[procNumber].mp_parent].mp_pid != 0 &&
                mproc[mproc[procNumber].mp_parent].mp_pid != 1)
          {
              path++;
              procNumber = mproc[procNumber].mp_parent;
          }
          if(path > maximumPath)
          {
            maximumPath = path;
            whoMax = mproc[procNumber].mp_pid;
          }
      }
    }
    *maxPath = maximumPath;
...
PUBLIC int do_findmaxpath()
{
  int maxPath = -1;
  pid_t who = -1;
  pid_t ignore = mm_in.m1_i1;
  maxpath(&maxPath, &who, ignore);
  return maxPath;
}

PUBLIC int do_whomaxpath()
{
  int maxPath = -1;
  pid_t who = -1;
  pid_t ignore = mm_in.m1_i1;
